...
class MapInterfacePageBodyState extends State<MapInterfacePageBody> {
  ...
  void _addLayerAndSource() async {
    mapboxMap?.style.styleSourceExists("source").then((value) async {
      if (!value) {
        var source = await rootBundle.loadString('assets/assets/source.json');
    ...
    mapboxMap?.style.styleLayerExists("points").then((value) async {
      if (!value) {
        var layer = await rootBundle.loadString('assets/assets/point_layer.json');
        mapboxMap?.style.addStyleLayer(layer, null);
      }
    });
  }
  ...
  */
  ...
  Widget build(BuildContext context) {
    final MapWidget mapWidget =
        MapWidget(key: ValueKey("mapWidget"), onMapCreated: _onMapCreated, resourceOptions: ResourceOptions(accessToken: 'pk.eyJ1IjoibWlrZW9sc2VuIiwiYSI6ImNsanllbjVybTAzYTczbHFnNGdxc2hoY2kifQ.w1MWWZd8X2Ofk1uKWFqhJA'));
