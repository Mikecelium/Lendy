...
class NewRental {
  ...
  final double rental_totalprice;
  final int rental_id;
  ...
  @override
  List<Object> get props => [rental_id, rental_timestamp, rental_title, rental_identifier, rental_totalprice, rentalStart, rentalEnd];

  // toJson method
  Map<String, dynamic> toJson() {
    return {
      'rental_id': rental_id,
      'rental_timestamp': rental_timestamp.toIso8601String(),
      'rental_title': rental_title,
      'rental_identifier': rental_identifier,
      'rental_totalprice': rental_totalprice,
  ...
  // fromJson method
  factory NewRental.fromJson(Map<String, dynamic> json) {
    return NewRental(
      rental_id: json['rental_id'],
      rental_timestamp: DateTime.parse(json['rental_timestamp']),
      rental_title: json['rental_title'],
      rental_identifier: json['rental_identifier'],
      rental_totalprice: json['rental_totalprice'],
      rentalStart: DateTime.parse(json['rentalStart']),
      rentalEnd: DateTime.parse(json['rentalEnd']),
    );
  }
...
class CustomData {
  ...
  static Future<CustomData> createWithRentalList({
    ...
    //required List<DocumentReference> rentalRefs, // Expecting DocumentReferences
  }) async {
    List<NewRental> rentals = [];

    //for (var ref in rentalRefs) {
      //DocumentSnapshot snapshot = await ref.get();
    //  if (snapshot.exists) {
    //    rentals.add(NewRental.fromJson(snapshot.data() as Map<String, dynamic>));
   //   }
   // }
...
class AnnotationClickListener extends OnCircleAnnotationClickListener {
  ...
  void onCircleAnnotationClick(CircleAnnotation annotation) {
    if (true /*annotation is CustomCircleAnnotation*/) {
      print("Annotation clicked: ${customDataList[int.parse(annotation.id)].title}");
...
SizedBox(
  ...
  child: ElevatedButton(
    ...
    onPressed: () {
      // Pass the annotation ID to the new page
      String annotationIdentifier = customDataList[int.parse(annotation.id)].identifier; // Assume annotation is non-null here
  ...
  await circleAnnotationManager?.deleteAll(); // Clear existing annotations
    ...
    for (var doc in snapshot.docs) {
      ...
      if(searchQuery.isEmpty || item.title.toLowerCase().contains(searchQuery.toLowerCase())) {
        ...
        List<NewRental> rentalList = item.rental_list;

        for (var rental in item.rental_list) {
      // Create a NewRental object from each element in item.rental_list.
      NewRental newRental = NewRental(
        rental_id: rental['rental_id'],
        rental_timestamp: DateTime.parse(rental['rental_timestamp']),
        rental_title: rental['rental_title'],
        rental_identifier: rental['rental_identifier'],
        rental_totalprice: rental['rental_totalprice'],
        rentalStart: DateTime.parse(rental['rentalStart']),
        rentalEnd: DateTime.parse(rental['rentalEnd']),
      );
      
      // Add the newly created NewRental object to the list.
      rentalList.add(newRental);
    }

        customDataList.add(CustomData(id: count , urlList: item.images, title: item.title, description: item.description, identifier: item.id,  price: item.rentalPrice, /*timestamp: DateTime.now())*/ rental_list: item.rental_list)) ;
...
Future<void> addCircleAnnotationsFromFirestore(String searchQuery, BuildContext context) async {
  try {
    await circleAnnotationManager?.deleteAll(); // Clear existing annotations
    var options1 = <CircleAnnotationOptions>[];
    var snapshot = await FirebaseFirestore.instance.collection('rentalItems').get();
    ...
    for (var doc in snapshot.docs) {
      RentalItem item = RentalItem.fromMap(doc.data() as Map<String, dynamic>);
        ...
        if (searchQuery.isEmpty || item.title.toLowerCase().contains(searchQuery.toLowerCase())) {
    //List<DocumentReference> rentalRefs = item.rental_list.map((e) => e as DocumentReference).toList();
    List<NewRental> rentalList = [];

        // Loop through each DocumentReference in the item's rental_list
//for (var rentalRef in rentalRefs) {
  // Use .get() on the DocumentReference to fetch the document
  //DocumentSnapshot rentalSnapshot = await rentalRef.get();
/*
  if (rentalSnapshot.exists) {
    Map<String, dynamic> rentalData = rentalSnapshot.data() as Map<String, dynamic>;
    // Now you have the rental data, construct a NewRental object
    NewRental newRental = NewRental(
      rental_id: rentalData['rental_id'],
      rental_timestamp: (rentalData['rental_timestamp'] as Timestamp).toDate(),
      rental_title: rentalData['rental_title'],
      rental_identifier: rentalData['rental_identifier'],
      rental_totalprice: (rentalData['rental_totalprice'] as num).toDouble(), // Ensure this handles int and double gracefully
      rentalStart: (rentalData['rentalStart'] as Timestamp).toDate(),
      rentalEnd: (rentalData['rentalEnd'] as Timestamp).toDate(),
    );

    rentalList.add(newRental);

    print(rentalList.first);
    print(rentalList.first);
    print(rentalList.first);
    print(rentalList.first);
    print(rentalList.first);
